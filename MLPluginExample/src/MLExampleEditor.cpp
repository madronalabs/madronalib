...
{
	MLPluginEditor* r = 0;	
	MLExampleEditor* pEd = new MLExampleEditor(pProcessor);
...
// ----------------------------------------------------------------
#pragma mark MLExampleEditor

MLExampleEditor::MLExampleEditor (MLPluginProcessor* const ownerProcessor) :
...
MLExampleEditor::~MLExampleEditor()
{
...
void MLExampleEditor::initialize (MLPluginProcessor* )
{
	...
	mpController = new MLExampleController(mpProcessor);
	mpView = new MLExampleView(mpProcessor, mpController);
...
void MLExampleEditor::resized()
{
	mpBorder->setSize(getWidth(), getHeight());
}

void MLExampleEditor::setWrapperFormat(int format)
{ 
