...
//const float ml::kTwoPi = 6.2831853071795864769252867f;
//const float ml::kPi = 3.1415926535897932384626433f;
const float kMLOneOverTwoPi = 1.0f / ml::kTwoPi;
...
public:
	...
	void setBandpass(float f, float q)
	{
		...
		mOmega = 2.0f * fsin1(ml::kPi * clampedFreq * invOver);		
	}
...
public:
	...
	{
		ka = ml::clamp(ml::kTwoPi*fa*mInvSr, 0.f, 0.25f);
		kb = ml::clamp(ml::kTwoPi*fb*mInvSr, 0.f, 0.25f);
	}
	void setAttackAndReleaseTimes(float tAttack, float tRelease)
	{
		ka = ml::clamp(ml::kTwoPi*(1.0f/tAttack)*mInvSr, 0.f, 0.25f);
		kb = ml::clamp(ml::kTwoPi*(1.0f/tRelease)*mInvSr, 0.f, 0.25f);
	}
...
public:
    ...
    inline float processSample()
    {
        ...
        x = fOmega + ml::fSignBit(mOmega32)*(kFlipOffset - fOmega - fOmega);
