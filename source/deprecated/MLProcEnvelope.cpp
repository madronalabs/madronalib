...
{	
	...
	for (int n=0; n<kFloatsPerDSPVector; ++n)
	{
		...
		// SSE here works on a single instance
		{
			...
			mCAttack =  ml::kTwoPi * invSr / ml::max(attackIn, kMinSegTime);
			mCDecay = ml::kTwoPi * invSr / ml::max(decay[n], kMinSegTime);
//			mCSustain = ml::kTwoPi * invSr / ();
			mCRelease = ml::kTwoPi * invSr / ml::max(release[n], kMinSegTime);
		}
		...
		crossedThresh = (ml::sign(mEnv - mEnvThresh) != ml::sign(mY1 - mEnvThresh));		
