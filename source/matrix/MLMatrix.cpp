...
// these can coexist with slower matrix-like Matrix methods that actually do range checking. 
...
// Matrix notTemporary(pOther, 4);
//
// NOTE this signal will not pass checkIntegrity()!
//
Matrix::Matrix(const Matrix* other, int slice) : 
mDataAligned(0),
mData(0)
{
	mRate = kToBeCalculated;
...
Matrix Matrix::getDims()
{
	if(mDepth > 1)
	{
		return Matrix{static_cast<float>(mWidth), static_cast<float>(mHeight), static_cast<float>(mDepth)};
	}
	else if(mHeight > 1)
	{
...
void Matrix::convolve5x1(const float kmm, const float km, const float k, const float kp, const float kpp)
{
	Matrix copy(*this);
	float* pIn = copy.getBuffer();
...
/*
 Vec2 Matrix::correctPeak(const int ix, const int iy, const float maxCorrect) const
 {		
