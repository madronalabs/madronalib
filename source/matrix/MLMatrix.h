...
public:
 ...
 //    const float getInterpolatedLinear(const Vec2& pos) const { return getInterpolatedLinear(pos.x(), pos.y()); }
	...
	// TODO should be explicit to prevent e.g. multiply by Matrix(2) when scale(2) was meant
...
inline Matrix transpose(const Matrix& x)
{
	...
	// return Matrix(float* pDataStart, int w, int h, int rowStride, int colStride);
	int yh = x.getWidth();
	int yw = x.getHeight();	
	Matrix y(yw, yh);	
	for(int j=0; j<yh; ++j)
