...
{
	MLPluginEditor* r = 0;	
	MLInstrumentEditor* pEd = new MLInstrumentEditor(pProcessor);
...
// ----------------------------------------------------------------
#pragma mark MLInstrumentEditor

MLInstrumentEditor::MLInstrumentEditor (MLPluginProcessor* const ownerProcessor) :
...
MLInstrumentEditor::~MLInstrumentEditor()
{
...
void MLInstrumentEditor::initialize (MLPluginProcessor* )
{
	...
	mpController = new MLInstrumentController(mpProcessor);
	mpView = new MLInstrumentView(mpProcessor, mpController);
...
void MLInstrumentEditor::resized()
{
	mpBorder->setSize(getWidth(), getHeight());
}

void MLInstrumentEditor::setWrapperFormat(int format)
{ 
